...
export default function CateringPage() {
  ...
  async function onSubmit(values: z.infer<typeof schema>) {
    const res = await fetch("/api/catering-request", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(values) });
    if (res.ok) {
              ...
              <Card key={p.id}>
                <CardHeader>
                  <div className="flex items-baseline justify-between">
                    ...
                    <span className="text-[--color-brand] font-semibold">{formatCurrency(p.pricePerPersonCents)} pp</span>
                  </div>
          ...
          <Card>
            ...
            <CardContent>
              <ul className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                {addOns.map((a) => (
                  <li key={a.id} className="flex items-center justify-between rounded-md border border-black/10 bg-white p-3 text-sm">
                    <span>{a.name}</span>
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                {env.catering.depositEnabled && (
                  <form action="/api/catering-deposit" method="POST">
                    <Button type="submit" variant="outline">Pay Deposit ({formatCurrency(env.catering.depositAmountCents)})</Button>
                  </form>
                )}
              </div>
              {!env.catering.depositEnabled && (
                <p className="text-xs text-muted">Optional deposit is disabled.</p>
              )}
            </CardContent>
          </Card>
        </div>
