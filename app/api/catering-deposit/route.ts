...
export async function POST() {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      mode: "payment",
      success_url: process.env.NEXT_PUBLIC_SITE_URL ? `${process.env.NEXT_PUBLIC_SITE_URL}/order/confirm?orderId=CATERING-${Date.now()}` : "http://localhost:3000/order/confirm",
      cancel_url: process.env.NEXT_PUBLIC_SITE_URL ? `${process.env.NEXT_PUBLIC_SITE_URL}/catering` : "http://localhost:3000/catering",
      line_items: [
        {
          price_data: {
